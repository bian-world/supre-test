...
import com.supretest.api.dto.datacount.ApiMethodUrlDTO;
...
@Service
public class StHashTreeService {
    @Resource
    ...
    private JSONObject setRefScenario(JSONObject element) {
        ...
        if(element.containsKey("step") && element.containsKey("script")){
        ...
        UiScenarioDTO uiScenarioDTO = extUiScenarioMapper.selectById(element.getString(ID));
        ...
        if (uiScenarioDTO != null && StringUtils.isNotEmpty(uiScenarioDTO.getScenarioDefinition())) {
            ...
            if (environmentEnable && StringUtils.isNotEmpty(uiScenarioDTO.getEnvironmentJson())) {
                element.put(ENV_MAP, JSON.parseObject(uiScenarioDTO.getEnvironmentJson(), Map.class));
            }
            if (StringUtils.equalsIgnoreCase(element.getString(REFERENCED), REF)) {
                element = JSON.parseObject(uiScenarioDTO.getScenarioDefinition(), Feature.DisableSpecialKeyDetect);
                element.put(REFERENCED, REF);
                element.put(NAME, uiScenarioDTO.getName());
            }
            element.put(ID, uiScenarioDTO.getId());
            element.put(ENV_ENABLE, environmentEnable);
            if (!element.containsKey(VARIABLE_ENABLE)) {
                element.put(VARIABLE_ENABLE, variableEnable);
            }
            this.setElement(element, uiScenarioDTO.getNum(), enable, uiScenarioDTO.getVersionName(), uiScenarioDTO.getVersionEnable());
        } else {
            if (StringUtils.equalsIgnoreCase(element.getString(REFERENCED), REF)) {
                element.put(ENABLE, false);
            }
