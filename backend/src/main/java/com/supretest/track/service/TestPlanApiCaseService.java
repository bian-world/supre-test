...
import com.supretest.base.mapper.TestPlanApiCaseMapper;
import com.supretest.base.mapper.TestPlanMapper;
import com.supretest.base.mapper.ext.ExtTestPlanApiCaseMapper;
...
import com.supretest.track.dto.TestPlanApiResultReportDTO;
...
public class TestPlanApiCaseService {
    ...
    public Pager<List<ApiTestCaseDTO>> relevanceList(int goPage, int pageSize, ApiTestCaseRequest request) {
        if (StringUtils.isNotBlank(request.getPlanId()) && !testPlanService.isAllowedRepeatCase(request.getPlanId())) { // 不允许重复关联
            List<String> ids = apiTestCaseService.selectIdsNotExistsInPlan(request.getProjectId(), request.getSubProjectId(), request.getPlanId());
