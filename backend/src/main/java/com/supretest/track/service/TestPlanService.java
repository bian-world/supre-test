...
import com.supretest.track.Factory.ReportComponentFactory;
import com.supretest.track.domain.ReportComponent;
import com.supretest.track.dto.*;
import com.supretest.track.request.testcase.PlanCaseRelevanceRequest;
import com.supretest.track.request.testcase.QueryTestPlanRequest;
import com.supretest.track.request.testplan.*;
...
public class TestPlanService {
    ...
    @Resource
    private TestPlanPublishedModuleMapper testPlanPublishedModuleMapper;
...
//                        performanceReportService.getReportLogs(reportId, resourceId);
    ...
    public void exportPlanPdfReport(String planId, HttpServletResponse response) throws IOException, InterruptedException, ExecutionException {
        response.reset();
        response.setContentType("application/pdf");
        response.addHeader("Content-Disposition", "attachment; filename=test");
        LogUtil.info("转pdf的页面地址为 {}", "http://localhost:"+ port +"/pdfPlanReport?planId=" + planId);
        byte[] bytes = HtmlTransferUtils.htmlToPdf("http://localhost:"+ port +"/pdfPlanReport?planId=" + planId);
    ...
    public TestPlanSimpleReportDTO getReport(String planId, TestPlanExecuteReportDTO testPlanExecuteReportDTO) {
        ...
        report.setStartTime(testPlan.getPlannedStartTime());
        report.setEndTime(testPlan.getPlannedEndTime());
        ...
        //测试计划中已发布模块数据
        List<TestPlanPublishedModuleDTO> publishedModule = testPlanPublishedModuleMapper.selectByPlanId(planId);
        report.setPublishedModule(publishedModule);
        return report;
    }
    ...
    public void publishModule(String planId, String moduleId, String flag){
        if(StringUtils.equals("add",flag)){
            TestPlanPublishedModule publishModule = new TestPlanPublishedModule();
            publishModule.setId(UUID.randomUUID().toString());
            publishModule.setCreateTime(System.currentTimeMillis());
            publishModule.setUpdateTime(System.currentTimeMillis());
            publishModule.setPlanId(planId);
            publishModule.setNodeId(moduleId);
            publishModule.setCreateUser(SessionUtils.getUserId());
            testPlanPublishedModuleMapper.insert(publishModule);
//            testPlanPublishedModuleMapper.
        }else if(StringUtils.equals("delete",flag)){
            TestPlanPublishedModuleExample example = new TestPlanPublishedModuleExample();
            TestPlanPublishedModuleExample.Criteria criteria = example.createCriteria().andPlanIdEqualTo(planId).andNodeIdEqualTo(moduleId);
         testPlanPublishedModuleMapper.deleteByExample(example);
        }else{ }
    }
