...
import com.supretest.excel.domain.ExcelErrData;
import com.supretest.excel.domain.ExcelResponse;
import com.supretest.excel.domain.TestCaseExcelData;
import com.supretest.excel.domain.TestCaseExcelDataFactory;
import com.supretest.excel.handler.FunctionCaseTemplateWriteHandler;
import com.supretest.excel.listener.TestCaseNoModelDataListener;
import com.supretest.excel.utils.EasyExcelExporter;
import com.supretest.excel.utils.FunctionCaseImportEnum;
...
import com.supretest.log.vo.track.TestCaseReference;
...
import com.supretest.track.issue.IssueFactory;
import com.supretest.track.request.testcase.*;
import com.supretest.track.request.testplan.LoadCaseRequest;
import com.supretest.xmind.XmindCaseParser;
import com.supretest.xmind.pojo.TestCaseXmindData;
import com.supretest.xmind.utils.XmindExportUtil;
...
public class TestCaseService {
    ...
    public void updateThirdPartyIssuesLink(EditTestCaseRequest testCase) {
        try {
            if (Class.forName("com.supretest.xpack.issue.service.XpackIssueService") != null) {
                Class clazz = Class.forName("com.supretest.xpack.issue.service.XpackIssueService");
    ...
    public List<TestCaseDTO> getReviewCase(QueryTestCaseRequest request) {
        ...
        newOrder.setName("update_time");
        newOrder.setType("desc");
        request.getOrders().add(0, newOrder);
        request.getOrders().forEach(order -> {
            order.setPrefix("test_case");
        });
...
//                        Collections.reverse(continueCaseList);
    ...
    public void download(String fileName, HttpServletResponse res) throws IOException {
        ...
        try (OutputStream outputStream = res.getOutputStream();
             BufferedInputStream bis = new BufferedInputStream(TestCaseService.class.getResourceAsStream("/com/supretest/xmind/template/" + fileName));) {
            int i = bis.read(buff);
            while (i != -1) {
    ...
    private List<List<Object>> generateTestCaseExcel(List<List<String>> headListParams, List<TestCaseExcelData> datas) {
        ...
        for (TestCaseExcelData model : datas) {
            ...
            for (String head : headList) {
                ...
                } else if (StringUtils.equalsAnyIgnoreCase(head, "Name", "用例名稱", "用例标题")) {
