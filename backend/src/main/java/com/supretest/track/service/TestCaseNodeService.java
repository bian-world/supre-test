...
import com.supretest.base.mapper.ext.ExtTestCaseReviewTestCaseMapper;
...
import com.supretest.dto.NodeNumDTO;
...
public class TestCaseNodeService extends NodeTreeService<TestCaseNodeDTO> {
    ...
    public String addNode(TestCaseNode node) {
        if(StringUtils.isEmpty(node.getZentaoTreeId())) {
            validateNode(node);
        }
    ...
    public List<TestCaseNodeDTO> getNodeTreeByProjectId(String projectId, String subProjectId) {
    ...
    public List<TestCaseNodeDTO> getAllNodeByProjectId(QueryNodeRequest request) {
        return getNodeTreeByProjectId(request.getProjectId(), null);
    }
    ...
    public List<TestCaseNodeDTO> getAllNodeByReviewId(QueryNodeRequest request) {
        ...
        return getNodeTreeByProjectId(projectId, null);
    }
    ...
    public Map<String, String> createNodes(List<String> nodePaths, String projectId) {
        List<TestCaseNodeDTO> nodeTrees = getNodeTreeByProjectId(projectId, null);
                ...
                for (TestCaseNodeDTO nodeTree : nodeTrees) {
                    if (StringUtils.equals(rootNodeName, nodeTree.getName())) {
                        ...
                        createNodeByPathIterator(itemIterator, "/" + rootNodeName, nodeTree,
                                pathMap, projectId, 2, StringUtils.isNotEmpty(project.getZentaoId())?true:false);
                    }
                }
            }
            if (!hasNode) {
                createNodeByPath(itemIterator, rootNodeName, null, projectId, 1, "", pathMap, StringUtils.isNotEmpty(project.getZentaoId())?true:false);
            }
        }
        return pathMap;
