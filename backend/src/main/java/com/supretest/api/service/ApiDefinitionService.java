...
import com.supretest.api.dto.automation.ReferenceDTO;
import com.supretest.api.dto.datacount.ApiDataCountResult;
import com.supretest.api.dto.definition.*;
import com.supretest.api.dto.definition.parse.ApiDefinitionImport;
import com.supretest.api.dto.definition.parse.ApiDefinitionImportParserFactory;
import com.supretest.api.dto.definition.parse.Swagger3Parser;
import com.supretest.api.dto.definition.request.assertions.document.DocumentElement;
...
import com.supretest.api.dto.swaggerurl.SwaggerUrlRequest;
import com.supretest.api.exec.api.ApiExecuteService;
import com.supretest.api.exec.utils.ApiDefinitionExecResultUtil;
import com.supretest.api.parse.ApiImportParser;
import com.supretest.base.domain.*;
import com.supretest.base.mapper.*;
import com.supretest.base.mapper.ext.*;
import com.supretest.commons.constants.*;
import com.supretest.commons.exception.MSException;
import com.supretest.commons.json.JSONSchemaToDocumentUtils;
import com.supretest.commons.json.JSONToDocumentUtils;
...
import com.supretest.dto.RelationshipEdgeDTO;
...
import com.supretest.log.vo.StatusReference;
import com.supretest.log.vo.api.DefinitionReference;
import com.supretest.notice.sender.NoticeModel;
import com.supretest.notice.service.NoticeSendService;
import com.supretest.service.*;
import com.supretest.track.request.testcase.ApiCaseRelevanceRequest;
...
public class ApiDefinitionService {
    ...
    private void checkNameExist(SaveApiDefinitionRequest request) {
        ...
        if(StringUtils.isNotEmpty(request.getSubProjectId())){
            criteria.andSubProjectIdEqualTo(request.getSubProjectId());
        }else{
            criteria.andSubProjectIdIsNull();
        }
...
//                    .andIdNotEqualTo(request.getId()).andVersionIdEqualTo(request.getVersionId());
    ...
    private List<ApiDefinition> getSameRequestWithName(SaveApiDefinitionRequest request) {
        ...
        if (request.getProtocol().equals(RequestType.HTTP)) {
            example.createCriteria()
                    ...
                    .andSubProjectIdEqualTo(request.getSubProjectId())
                    .andIdNotEqualTo(request.getId());
        } else {
            example.createCriteria()
                    .andStatusNotEqualTo("Trash")
    ...
    private ApiDefinitionWithBLOBs updateTest(SaveApiDefinitionRequest request) {
        ...
        test.setSubProjectId(request.getSubProjectId());
...
//                andVersionIdEqualTo(request.getVersionId());
        ...
        if (StringUtils.equals(test.getProtocol(), "HTTP")) {
            ...
            apiTestCaseService.updateByApiDefinitionId(ids, test.getPath(), test.getMethod(), test.getProtocol(), test.getSubProjectId());
        }
        //
...
//        checkAndSetLatestVersion(result.getRefId());
    ...
    private void _importCreate(List<ApiDefinition> sameRequest, ApiDefinitionMapper batchMapper, ApiDefinitionWithBLOBs apiDefinition,
        ...
        if (CollectionUtils.isEmpty(sameRequest)) { // 没有这个接口 新增
            ...
            if (!apiOp.isPresent()) {
                ...
                if (!StringUtils.equalsIgnoreCase(apiTestImportRequest.getPlatform(), ApiImportPlatform.Supretest.name())) {
    ...
    private void importApi(ApiTestImportRequest request, ApiDefinitionImport apiImport) {
        ...
        for (int i = 0; i < data.size(); i++) {
            ...
            item.setNum(num++);
            item.setSubProjectId(request.getSubProjectId()  );
     ...
     * @return
     */
    public List<ApiDataCountResult> countProtocolByProjectID(String projectId, String subProjectId) {
        return extApiDefinitionMapper.countProtocolByProjectID(projectId, subProjectId);
    }
     ...
     * @return
     */
    public long countByProjectIDAndCreateInThisWeek(String projectId, String subProjectId) {
        ...
        if (firstTime == null || lastTime == null) {
            ...
            return extApiDefinitionMapper.countByProjectIDAndCreateInThisWeek(projectId, subProjectId, firstTime.getTime(), lastTime.getTime());
        }
    }

    public List<ApiDataCountResult> countStateByProjectID(String projectId, String subProjectId) {
        return extApiDefinitionMapper.countStateByProjectID(projectId, subProjectId);
    }

    public List<ApiDataCountResult> countApiCoverageByProjectID(String projectId, String subProjectId) {
        return extApiDefinitionMapper.countApiCoverageByProjectID(projectId, subProjectId);
    }
...
//        extApiDefinitionMapper.removeToGcByExample(example);
    ...
    public List<SwaggerTaskResult> getSwaggerScheduleList(String projectId, String subProjectId) {
        List<SwaggerTaskResult> resultList = extSwaggerUrlScheduleMapper.getSwaggerTaskList(projectId, subProjectId);
        int dataIndex = 1;
    ...
    public ApiExportResult export(ApiBatchRequest request, String type) {
        ...
        if (StringUtils.equals(type, "SUPRETEST")) { //  导出为 SupreTest 格式
            MockConfigService mockConfigService = CommonBeanFactory.getBean(MockConfigService.class);
            ...
            ((MsApiExportResult) apiExportResult).setVersion(System.getenv("VERSION"));
    ...
    public List<ApiDefinition> selectEffectiveIdByProjectId(String projectId, String subProjectId) {
        return extApiDefinitionMapper.selectEffectiveIdByProjectId(projectId, subProjectId);
    }
    ...
    public long countEffectiveByProjectId(String projectId, String subProjectId) {
        if (StringUtils.isEmpty(projectId)) {
            return 0;
        } else {
            ApiDefinitionExample example = new ApiDefinitionExample();
            example.createCriteria().andProjectIdEqualTo(projectId).andStatusNotEqualTo("Trash").andLatestEqualTo(true).andSubProjectIdEqualTo(subProjectId);
            return apiDefinitionMapper.countByExample(example);
        }
    }

    public long countQuotedApiByProjectId(String projectId, String subProjectId) {
        return extApiDefinitionMapper.countQuotedApiByProjectId(projectId, subProjectId);
    }
