...
//        T nodeTree = (T) new TreeNodeDTO();
    ...
    protected void createNodeByPathIterator(Iterator<String> pathIterator, String path, T treeNode,
                                            Map<String, String> pathMap, String projectId, Integer level, Boolean isZentaoProject) {
        ...
        if (children == null || children.isEmpty() || !pathIterator.hasNext()) {
            ...
            if (pathIterator.hasNext()) {
                createNodeByPath(pathIterator, pathIterator.next().trim(), treeNode, projectId, level, path, pathMap, isZentaoProject);
            }
            return;
        }
        ...
        for (T child : children) {
            if (StringUtils.equals(nodeName, child.getName())) {
                ...
                createNodeByPathIterator(pathIterator, path + "/" + child.getName(),
                        child, pathMap, projectId, level + 1, isZentaoProject);
            }
            ;
        }
        ...
        if (!hasNode) {
            createNodeByPath(pathIterator, nodeName, treeNode, projectId, level, path, pathMap, isZentaoProject);
        }
    ...
    protected void createNodeByPath(Iterator<String> pathIterator, String nodeName,
                                    ...
                                    String rootPath, Map<String, String> pathMap, Boolean isZentaoProject) {
        ...
        if (pathMap.get(path.toString()) != null) {
            ...
            level++;
        } else if(isZentaoProject){
            MSException.throwException(Translator.get("模块不存在"));
        }
        else {
