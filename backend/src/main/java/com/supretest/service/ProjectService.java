...
import com.supretest.api.dto.DeleteAPITestRequest;
...
import com.supretest.api.service.ApiTestDelService;
...
import com.supretest.base.mapper.ext.ExtProjectVersionMapper;
import com.supretest.base.mapper.ext.ExtUserGroupMapper;
import com.supretest.base.mapper.ext.ExtUserMapper;
...
import com.supretest.dto.*;
import com.supretest.i18n.Translator;
import com.supretest.job.sechedule.CleanUpReportJob;
import com.supretest.job.sechedule.ProjectWeekPlanProgressJob;
...
import com.supretest.performance.request.DeleteTestPlanRequest;
...
import com.supretest.track.issue.AbstractIssuePlatform;
import com.supretest.track.issue.JiraPlatform;
import com.supretest.track.issue.TapdPlatform;
...
import com.supretest.track.service.*;
...
public class ProjectService {
    ...
    @Resource
    private ZendaoProjectMapper zendaoProjectMapper;
    ...
    public Project addProject(AddProjectRequest project) {
        ...
        if (!StringUtils.equals(project.getPlatform(),"ZenTao")) {
    ...
    public List<ProjectDTO> getAllProject() {
        return extProjectMapper.getAllProject();
    }

    public List<ProjectWeekCaseDTO> getProjectWeekCase(String projectId, String subProjectId) {
        return extProjectMapper.getProjectWeekCase(projectId, subProjectId);
    }

    public int insertProjectWeekPlanProgressesList() {
        List<ProjectDTO> projectDTOList = getAllProject();
        List<ProjectWeekPlanProgressDTO> projectWeekPlanProgressDTOList = new ArrayList<ProjectWeekPlanProgressDTO>();
        for(ProjectDTO projectDTO: projectDTOList){
            QueryTestPlanRequest request = new QueryTestPlanRequest();
            request.setProjectId(projectDTO.getId());
            List<TestPlanDTOWithMetric> testPlanDTOWithMetriclist = testPlanService.listTestPlan(request);
            for(TestPlanDTOWithMetric testPlanDTOWithMetric: testPlanDTOWithMetriclist){
                if(!testPlanDTOWithMetric.getStatus().equals("Finished") || !testPlanDTOWithMetric.getStatus().equals("Archived")) {
                    projectWeekPlanProgressDTOList.add(new ProjectWeekPlanProgressDTO(
                            String.valueOf(UUID.randomUUID()), testPlanDTOWithMetric.getProjectId(),testPlanDTOWithMetric.getSubProjectId(),
                            testPlanDTOWithMetric.getId(), testPlanDTOWithMetric.getName(),
                            testPlanDTOWithMetric.getPassRate(), testPlanDTOWithMetric.getPassed(),
                            testPlanDTOWithMetric.getTotal(), String.valueOf(System.currentTimeMillis())
                    ));
                }
            }

        }
        return extProjectMapper.insertProjectWeekPlanProgress(projectWeekPlanProgressDTOList);
    }

    public Map<String, List<?>> getProjectPlanProgress(String projectId, String subProjectId) {
        List<ProjectWeekPlanProgressDTO> projectWeekPlanProgressesList = extProjectMapper.getProjectPlanProgress(projectId, subProjectId);
        QueryTestPlanRequest request = new QueryTestPlanRequest();
        ...
        List<TestPlanDTOWithMetric> testPlanDTOWithMetriclist = testPlanService.listTestPlan(request);
        for(TestPlanDTOWithMetric testPlanDTOWithMetric: testPlanDTOWithMetriclist){
            projectWeekPlanProgressesList.add(new ProjectWeekPlanProgressDTO(
                    String.valueOf(UUID.randomUUID()), testPlanDTOWithMetric.getProjectId(), testPlanDTOWithMetric.getSubProjectId(),
                    testPlanDTOWithMetric.getId(), testPlanDTOWithMetric.getName(),
                    testPlanDTOWithMetric.getPassRate(), testPlanDTOWithMetric.getPassed(),
                    testPlanDTOWithMetric.getTotal(), String.valueOf(new SimpleDateFormat("M月d日").format(new Date())))
            );
        }
//        String msg = "[{ 'id': 'df87b61a-e864-11ec-b316-6668d4ee2e1b', 'projectId': 'eb965e8f-95ba-40cd-89c1-5f9ac615f1f4', 'planId': 'a0d90fcc-7793-4cd9-bffb-48a52eb81575', 'prepareNum': 2, 'planCaseTotal': 4, 'createDay': '6月10日' }, { 'id': '7hdi3jdi-e864-11ec-b316-6668d4ee2des', 'projectId': 'eb965e8f-95ba-40cd-89c1-5f9ac615f1f4', 'planId': 'b0d90fcc-7793-4cd9-bffb-48a52eb81576', 'prepareNum': 1, 'planCaseTotal': 2, 'createDay': '6月8日' }, { 'id': '9dneu6cg-e864-11ec-b316-6668d4ee2e1b', 'projectId': 'eb965e8f-95ba-40cd-89c1-5f9ac615f1f4', 'planId': 'a0d90fcc-7793-4cd9-bffb-48a52eb81575', 'prepareNum': 2, 'planCaseTotal': 4, 'createDay': '6月9日'}]";
        JSONArray data = JSONArray.parseArray(JSON.toJSONString(projectWeekPlanProgressesList));
        Map<String, List<?>> dataMap = new HashMap<String, List<?>>();
        //数据分组算法
        for (int i = 0; i < data.size(); i++) {
            JSONObject json = (JSONObject)data.get(i);
            List<Object> tempList = (List<Object>) dataMap.get(json.get("planName"));

            if(tempList == null) { //说明是第一次，则创建list
                tempList = new ArrayList<>();
                tempList.add(json);
                dataMap.put(json.get("planName").toString(), tempList);//放入map中
            }else {
                tempList.add(json);
            }
        }
        ...
        for(TestPlanDTOWithMetric testPlanDTOWithMetric: testPlanDTOWithMetriclist){
            if (testPlanDTOWithMetric.getStatus().equals("Finished") || testPlanDTOWithMetric.getStatus().equals("Archived") ||
                    testPlanDTOWithMetric.getStatus().equals("Prepare")){
                dataMap.remove(testPlanDTOWithMetric.getName());
            }
        }
    ...
    public void projectWeekProgressSchedule(Boolean enable) {
        String resourceId = "ProjectProgressSchedule";
        Schedule schedule = scheduleService.getScheduleByResource(resourceId, ScheduleGroup.PROJECT_PLAN_PROGRESS.name());
        if (schedule != null && StringUtils.isNotBlank(schedule.getId())) {
            ...
            scheduleService.addOrUpdateCronJob(schedule,
                    ProjectWeekPlanProgressJob.getJobKey(resourceId),
                    ProjectWeekPlanProgressJob.getTriggerKey(resourceId),
                    ProjectWeekPlanProgressJob.class);
        } else {
            ScheduleRequest request = new ScheduleRequest();
            request.setName("ProjectProgressSchedule");
            request.setResourceId(resourceId);
            request.setKey(resourceId);
            //        request.setProjectId("");
            request.setWorkspaceId(SessionUtils.getCurrentWorkspaceId());
            request.setEnable(Boolean.TRUE);
            request.setUserId(SessionUtils.getUserId());
            request.setGroup(ScheduleGroup.PROJECT_PLAN_PROGRESS.name());
            request.setType(ScheduleType.CRON.name());
            // 每天凌晨1点执行同步测试进度
            request.setValue("0 0 1 * * ?");
            request.setJob(ProjectWeekPlanProgressJob.class.getName());
    ...
    public List<ZendaoProject> getZendaoProjects(String projectId) {
        ZendaoProjectExample zendaoProjectExample = new ZendaoProjectExample();
        if (StringUtils.isNotBlank(projectId)) {
            zendaoProjectExample.createCriteria().andProductIdEqualTo(projectId);
        }
        return zendaoProjectMapper.selectByExample(zendaoProjectExample);
    }

    public  Boolean  isZendaoProject(String projectId) {
        ZendaoProjectExample zendaoProjectExample = new ZendaoProjectExample();
        if (StringUtils.isNotBlank(projectId)) {
            zendaoProjectExample.createCriteria().andProductIdEqualTo(projectId).andZendaoProductIdIsNotNull();
        }
        return zendaoProjectMapper.selectByExample(zendaoProjectExample).size() > 1;
    }
