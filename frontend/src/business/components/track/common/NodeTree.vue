    ...
    <el-tree
      ...
      <span class="custom-tree-node father" @click="handleNodeSelect(node)">
        ...
        <span class="count-title" v-if="isDisplay!=='relevance'">
          <span style="color: #63B8FF">{{ data.caseNum }}</span>
        </span>
        <span v-if="!disabled" class="node-operate child">
          <el-tooltip
            v-if="data.id !== 'root' && data.name !== defaultLabel && data.isTreeNodeEditable === true "
            class="item"
            effect="dark"
          ...
          <el-tooltip
            v-if="data.name === defaultLabel && data.level !==1 && data.isTreeNodeEditable === true"
            v-permission="updatePermission"
            class="item"
            effect="dark"
            :open-delay="200"
    ...
    init() {
      ...
      if(this.treeNodes.length>0) {
        this.extendTreeNodes.unshift({
          ...
          "caseNum": num > 0 ? num : "",
          "isTreeNodeEditable": this.treeNodes[0].isTreeNodeEditable
        });
      }
