...
export default {
  ...
  methods: {
    init() {
      ...
      let selectSubProjectId = this.subProjectId;
      if (!selectProjectId) {
        return;
      }
      ...
      this.$get("/track/relevance/count/" + selectProjectId + "/" + selectSubProjectId, response => {
        this.relevanceCountData = response.data;
      });

      this.$get("/track/case/bar/" + selectProjectId + "/" + selectSubProjectId, response => {
