        ...
        <ms-table-column
          ...
          :label="$t('commons.trigger_mode.schedule')">
           <template v-slot="scope">
            ...
            <span v-else>
             <el-link @click.stop="scheduleTask(scope.row)" :disabled="!hasSchedulePermission" style="color:#63B8FF;">{{
                 $t('schedule.not_set')
               }}</el-link>
            </span>
          </template>
  ...
  methods: {
    ...
    intoPlan(row, column, event) {
      if (column.label !== this.$t('commons.operating')) {
        this.$router.push('/plan/view/' + row.id);
      }
    },
    filter(filters) {
