  ...
  <test-plan-report-container id="api" :title="$t('test_track.report.analysis_api')">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane v-if="resultEnable" :label="$t('test_track.report.test_result')" name="first">
        <api-result :api-result="report.apiResult" :is-pdf="isPdf"/>
      </el-tab-pane>
...
<!--      </el-tab-pane>-->
      ...
      <el-tab-pane style="min-height: 500px" name="fifth" v-if="allEnable && !isPdf">
        <template v-slot:label>
...
export default {
  ...
  props: [
    'report', 'planId', 'isTemplate', 'isShare', 'shareId', 'isDb','isPdf'
  ],
  computed: {
    resultEnable() {
