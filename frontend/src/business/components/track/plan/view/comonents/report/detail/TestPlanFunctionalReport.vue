  ...
  <test-plan-report-container id="functional" :title="$t('test_track.report.analysis_functional')">
    <el-tabs v-model="activeName" @tab-click="handleClick">
      <el-tab-pane v-if="resultEnable" :label="$t('test_track.report.test_result')" name="first">
        <functional-result :is-template="isTemplate" :share-id="shareId" :is-share="isShare" :is-pdf="isPdf" :is-db="isDb" :report="report"  :plan-id="planId"/>
      </el-tab-pane>
      <el-tab-pane v-if="failureEnable && !isPdf" name="second">
...
<!--      </el-tab-pane>-->
      <el-tab-pane name="fourth" v-if="allEnable && !isPdf">
        <template v-slot:label>
...
export default {
  ...
  props: [
    'report','planId', 'isTemplate', 'isShare', 'shareId', 'config', 'isDb','isPdf'
  ],
  computed: {
    resultEnable() {
