...
import {getCurrentProjectID, getCurrentSubProjectID, hasLicense} from "@/common/js/utils";
...
export default {
  ...
  methods: {
    ...
    getTestPlans() {
      let condition = {projectId: getCurrentProjectID()}
      let subProjectId = getCurrentSubProjectID();
      if(subProjectId){
        condition = {...condition,subProjectId: subProjectId}
      }
      this.$post('/test/plan/list/all', condition, response => {
    ...
    changePlan(plan) {
      this.currentPlan = plan;
      this.$router.push('/plan/view/' + plan.id);
    },
    handleSelect(key) {
