...
<!--      </ms-table-column>-->
        ...
        <ms-table-column
          ...
          :field="item"
          :fields-width="fieldsWidth"
          :label="$t('commons.case_name')"
          min-width="120"
        />

        <ms-table-column :label="$t('test_track.case.case_desc')" prop="desc" :field="item" min-width="100px">
          <template v-slot:default="scope">
            <el-link @click.stop="getCase(scope.row.id)" style="color:#63B8FF;">{{ $t('commons.preview') }}</el-link>
          </template>
        </ms-table-column>
  ...
  methods: {
    ...
    getData() {
      ...
      if (this.projectId) {
        ...
        if(this.subProjectId) {
          this.condition.subProjectId = this.subProjectId;
        }
        this.$emit('setCondition', this.condition);
    ...
    exportTestCase(exportType) {
      ...
      this.page.result = this.$request(config).then(response => {
        const filename = "Supretest_case_" + this.projectName + fileNameSuffix;
