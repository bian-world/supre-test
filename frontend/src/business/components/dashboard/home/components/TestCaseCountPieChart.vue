<template>
  <el-card class="table-card" body-style="padding:10px;" v-loading="result.loading">
    <div slot="header">
      <span class="title">
        {{ $t('dashboard.test_case_count') }}
      </span>
    </div>
    <el-container>
      <el-aside width="45%">
        <st-rose-pie-chart style="width:100%; height:340px" :data="trackCountData.data" name=""/>
      </el-aside>
      <el-main style="padding-left: 0px;padding-right: 0px;padding-top: 10%">
        <span style="font-size: 12px"> {{
            $t('api_test.home_page.api_details_card.this_week_add')
          }} {{ trackCountData.thisWeekAddedCount }}个</span><br/><br/>
        <el-row align="middle" type="flex" justify="space-around"
          ...
          <el-col :span="9"
            ...
            <span style="margin-left: 20%;font-size: 30px;color:#0099ff">{{ trackCountData.reviewRage }}</span>
            <span style="font-size: 12px;color:#0099ff">%</span><br/>
            <span style="font-size: 12px;margin-left: 20%">{{ $t('test_track.home.review_rate') }}</span>

          </el-col>
          <el-col :span="5"
                  style="padding: 5px;border-right-style: solid;border-right-width: 0px;border-right-color: #ECEEF4;">
            <span style="margin-left: 20%;font-size: 20px;color:#bcc0c3">{{ trackCountData.prepareCount }}</span><br/>
            <span style="font-size: 12px;margin-left: 20%">{{ $t('test_track.review.prepare') }}</span>
          </el-col>
          <el-col :span="5"
                  style="padding: 5px;border-right-style: solid;border-right-width: 0px;border-right-color: #ECEEF4;">
            <span style="margin-left: 20%;font-size: 20px;color:#db3a52">{{ trackCountData.unPassCount }}</span><br/>
            <span style="font-size: 12px;margin-left: 20%">{{ $t('test_track.review.un_pass') }}</span>
          </el-col>
          <el-col :span="5" style="padding: 5px;">
            <span style="margin-left: 20%;font-size: 20px;color:#34b34a">{{ trackCountData.passCount }}</span><br/>
            <span style="font-size: 12px;margin-left: 20%">{{ $t('test_track.review.pass') }}</span>
          </el-col>
        </el-row>
        <!--        </div>-->
      </el-main>
...
export default {
  ...
  // watch: {
  //   trackCountData(newData) {
  //     this.review_rate = newData.reviewRage.split('%')[0]
  //     console.log(this.review_rate)
  //   }
  // },
  methods: {
