...
<!--          <ms-table-column-->
<!--            v-if="item.id == 'num'" prop="customNum"-->
<!--            label="ID"-->
<!--            sortable-->
<!--            :fields-width="fieldsWidth"-->
<!--            min-width="120px">-->
...
<!--          </ms-table-column>-->
          ...
          <ms-table-column  v-if="item.id == 'updateTime'" :prop="item.id"
                            :field="item"
                            :fields-width="fieldsWidth"
...
<!--        </template>-->
    ...
    <ms-schedule-maintain ref="scheduleMaintain"  :request="runRequest"/>
  </div>
...
import {UI_AUTOMATION_LIST} from "@/common/js/constants";
...
export default {
  ...
  components: {
    ...
    BatchMove: () => import("@/business/components/track/case/components/BatchMove"),
    EnvironmentSelect: () => import("@/business/components/api/definition/components/environment/EnvironmentSelect"),
    BatchEdit: () => import("@/business/components/track/case/components/BatchEdit"),
    PlanStatusTableItem: () => import("@/business/components/track/common/tableItems/plan/PlanStatusTableItem"),
    PriorityTableItem: () => import("@/business/components/track/common/tableItems/planview/PriorityTableItem"),
    ...
    MsTableMoreBtn: () => import("@/business/components/api/automation/scenario/TableMoreBtn"),
    ...
    MsTag: () => import("@/business/components/common/components/MsTag"),
    MsApiReportDetail: () => import("@/business/components/api/automation/report/ApiReportDetail"),
    SysnApiReportDetail: () => import("@/business/components/api/automation/report/SysnApiReportDetail"),
    MsScenarioExtendButtons: () => import("@/business/components/api/automation/scenario/ScenarioExtendBtns"),
    MsTestPlanList: () => import("@/business/components/api/automation/scenario/testplan/TestPlanList"),
    MsTableOperatorButton: () => import("@/business/components/common/components/MsTableOperatorButton"),
    MsRunMode: () => import("@/business/components/api/automation/scenario/common/RunMode"),
  ...
  data() {
    return {
      projectName: "",
      result: {},
      tableHeaderKey: "UI_AUTOMATION",
      type: UI_AUTOMATION_LIST,
      fields: getCustomTableHeader('UI_AUTOMATION'),
      fieldsWidth: getCustomTableWidth('UI_AUTOMATION'),
      ...
      unTrashOperators: [
        ...
        {
          tip: this.$t('commons.timer'),
          exec: this.openSchedule,
          icon: "el-icon-time",
          permissions: ['PROJECT_API_SCENARIO:READ+DELETE']
        },
        {
  ...
  methods: {
    getPreScenario(preScenarioId){
      return new Promise((resolve) => {
        this.$get('/ui/scenario/getUiScenario/preScenario/' + preScenarioId, response => {
          ...
          this.currentScenario.scenarioDefinition.hashTree = [...obj, ...this.currentScenario.scenarioDefinition.hashTree];
          resolve();
    ...
    run(row) {
      ...
      this.getUiScenario(row.id).then(async () => {
        let scenarioStep = this.currentScenario.scenarioDefinition;
        if(scenarioStep.preScenarioId) {
          await this.getPreScenario(scenarioStep.preScenarioId)
    ...
    getUiScenario(scenarioId) {
      return new Promise((resolve) => {
        this.result = this.$get("/ui/scenario/getUiScenario/" + scenarioId, response => {
          if (response.data) {
            this.currentScenario = response.data;
            this.currentScenario.clazzName = TYPE_TO_C.get("UiScenario");
    ...
    search(projectId) {
      ...
      if (subProjectId) {
        this.condition.subProjectId = subProjectId;
      }
      ...
      let url = "/ui/scenario/list/" + this.currentPage + "/" + this.pageSize;
    ...
    edit(row) {
      ...
      // data.preScenarioId = data.
      this.$emit('edit', data);
    },
    addSon(row){
      let data = JSON.parse(JSON.stringify(row));
      let sonData = {"fatherPageId": data.id, "moduleId": data.moduleId, "subProjectId": data.subProjectId}
      this.$emit('edit', sonData);
    },
    reductionApi(row) {
    ...
    remove(row) {
      if (!this.trashEnable) {
        let alertMsg = this.$t('ui.delete_case_confirm') + " [" + row.name + "] ?";
        this.$alert(alertMsg, '', {
          confirmButtonText: this.$t('commons.confirm'),
          cancelButtonText: this.$t('commons.cancel'),
          callback: (action) => {
            if (action === 'confirm') {
              this.$post('/ui/scenario/delete/' + row.id, {},() => {
                this.$success(this.$t('commons.delete_success'));
                this.search();
              });
            }
      ...
      this.$get('project/get/' + this.projectId, response => {
        ...
        if (project) {
          ...
          this.result = this.$post("/api/automation/export", param, response => {
            ...
            downloadFile("Supretest_Scenario_" + this.projectName + ".json", JSON.stringify(obj));
          });
        }
      });
    },
