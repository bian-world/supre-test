...
<!--          }}</span>-->
      ...
      <template v-slot:debugStepCode>
        <span  class="ms-ui-screenshot" @click="showImage" style="font-size: 12px; margin-right: 20px" v-if="!loading &&!request.testing  && request.requestResult[0] && request.requestResult[0].responseResult.headers">
           查看截图
          <el-image-viewer  v-if="showImg"  :initialIndex="initialIndex" :zIndex="zIndex" :on-close="closeViewer" :url-list="imgList" />
         </span>
        ...
        <span class="ms-step-debug-code"
              ...
              v-else-if="!loading &&!request.testing  && request.requestResult[0] && request.requestResult[0].responseResult.responseCode">
      ...
      <template v-slot:request>
        <legend style="width: 100%;">
          <div v-if="!ifFromVariableAdvance">
            <ui-automation-operator-step :is-customize-req="isCustomizeReq" :request="request" @setDomain="setDomain"/>
            <br>
            <p class="tip">{{ $t('api_test.definition.request.req_param') }} </p>
            <ui-automation-operator-form
              v-if="request.type==='StWebDriverSampler'"
...
import UiAutomationOperatorStep from "@/business/components/ui/uiautomation/UiAutomationOperatorStep";
import UiAutomationOperatorForm from "@/business/components/ui/uiautomation/UiAutomationOperatorForm";
...
export default {
  ...
  components: {
    ElImageViewer,
    UiAutomationOperatorForm,
    UiAutomationOperatorStep,
    ...
    ApiBaseComponent: () => import("@/business/components/api/automation/scenario/common/ApiBaseComponent"),
    ApiResponseComponent: () => import("@/business/components/api/automation/scenario/component/ApiResponseComponent"),
    MsSqlBasisParameters: () => import("@/business/components/api/definition/components/request/database/BasisParameters"),
    MsTcpFormatParameters: () => import("@/business/components/api/definition/components/request/tcp/TcpFormatParameters"),
    MsDubboBasisParameters: () => import("@/business/components/api/definition/components/request/dubbo/BasisParameters"),
    MsApiRequestForm: () => import("@/business/components/api/definition/components/request/http/ApiHttpRequestForm"),
    MsRequestResultTail: () => import("@/business/components/api/definition/components/response/RequestResultTail"),
    MsRun: () => import("@/business/components/api/definition/components/Run"),
  ...
  methods: {
    ...
    showImage(){
      let responseImg = this.request.requestResult[0].responseResult.headers;
      if(responseImg) {
        let img = "data:image/png;base64," + responseImg;
        this.imgList[0] = img ;
        this.showImg = true;
      }
    },
