  ...
  <ms-container>
    <ms-aside-container>
      <div @click="showAll">
        ...
        <el-form :model="currentCase" label-position="right" label-width="80px" size="small" :rules="rules"
                 ref="currentCase">
          <!-- 基础信息 -->
          <el-form-item :label="$t('commons.name')" prop="name">
            <el-input class="ms-scenario-input" size="small" v-model="currentCase.name"/>
          </el-form-item>

          <el-form-item label="所属项目" prop="subProject">

            <el-select filterable ref="select"
                       v-model="currentCase.subProjectId"
                       :clearable="true"
                       placeholder="选择项目"
            >
            ...
            <!--          <ms-select-tree :disabled="readOnly" :data="treeNodes" :defaultKey="form.subProject" :obj="moduleObj"-->
            <!--                          @getValue="setModule"  clearable formsize="small"/>-->
          </el-form-item>

          <el-form-item :label="$t('test_track.module.module')" prop="moduleId">
            <ms-select-tree size="small" :data="moduleOptions" :defaultKey="currentCase.moduleId"
                            @getValue="setModule" :obj="moduleObj" clearable />
          </el-form-item>


          <el-form-item :label="$t('commons.status')" prop="status">
            <el-select class="ms-scenario-input" size="small" v-model="currentCase.status">
          ...
          <el-form-item :label="$t('api_test.definition.request.responsible')" prop="principal">
            <el-select v-model="currentCase.principal"
          ...
          <el-form-item :label="$t('test_track.case.priority')" prop="level">
            <el-select class="ms-scenario-input" size="small" v-model="currentCase.level">
...
<!--            <ms-input-tag :currentCase="currentCase" ref="tag"/>-->
<!--          </el-form-item>-->
          ...
          <el-form-item label="ID" prop="customNum" v-if="customNum">
            <el-input v-model.trim="currentCase.customNum" size="small"></el-input>
          </el-form-item>
...
<!--          </el-col>-->
          ...
          <el-col :span="6" >
                      <el-checkbox v-model="enablePreScenario" class="ms-col-one ms-font" @change="clearPreScenario">
                        <span style="font-size: 13px; margin-right: 4px" >前置场景</span></el-checkbox>
            <el-select filterable v-if="enablePreScenario" ref="select"
                       v-model="currentCase.preScenarioId"
                       :clearable="true"
                       placeholder="请选择"
                       size="mini"
                       @change="setPreScenario"
            >
              <el-option v-for="item in scenarioList" :key="item.id" :label="item.name" :value="item.id"></el-option>
...
<!--            <el-tooltip class="item" effect="dar k" :content="$t('commons.refresh')" placement="top-start">-->
...
<!--                                @compare="compare" @checkout="checkout" @create="create" @del="del"/>-->
                      ...
                      <el-button type="warning" size="mini" style="margin-left: 10px" @click="handleStep($t('ui.add_step'))"><i class="el-icon-plus" />{{$t('ui.add_step')}}</el-button>
...
<!--              <i class="el-icon-plus"/>{{$t('api_test.automation.api_scenario')}}-->
<!--              <el-dropdown-menu slot="dropdown">-->
<!--                <el-dropdown-item :command="$t('api_test.automation.customize_req')">{{$t('api_test.automation.customize_req')}}</el-dropdown-item>-->
<!--                <el-dropdown-item :command="$t('api_test.automation.api_list_import')">{{ $t('api_test.automation.api_list_import')}}</el-dropdown-item>-->
<!--                <el-dropdown-item :command="$t('api_test.automation.scenario_import')">{{ $t('api_test.automation.scenario_import')}}</el-dropdown-item>-->
<!--              </el-dropdown-menu>-->
<!--            </el-dropdown>-->

<!--            <el-dropdown split-button type="success"  size="mini" style="margin-left: 10px"-->
<!--                         @command="handleStep">-->

<!--              <i class="el-icon-plus"/>{{$t('api_test.automation.controller')}}-->
<!--              <el-dropdown-menu slot="dropdown">-->
<!--                <el-dropdown-item :command="$t('api_test.automation.wait_controller')">{{ $t('api_test.automation.wait_controller')}}</el-dropdown-item>-->
<!--                <el-dropdown-item :command="$t('api_test.automation.loop_controller')">{{$t('api_test.automation.loop_controller')}}</el-dropdown-item>-->
<!--                <el-dropdown-item :command="$t('api_test.automation.transcation_controller')">{{$t('api_test.automation.transcation_controller')}}</el-dropdown-item>-->
<!--                <el-dropdown-item :command="$t('api_test.automation.if_controller')">{{ $t('api_test.automation.if_controller') }}</el-dropdown-item>-->
<!--              </el-dropdown-menu>-->
<!--            </el-dropdown>-->

<!--            <el-button type="warning" size="mini" style="margin-left: 10px" @click="handleStep($t('api_test.automation.customize_script'))"><i class="el-icon-plus" />{{$t('api_test.automation.customize_script')}}</el-button>-->
<!--          </el-col>-->
<!--        </el-row>-->
      </div>
...
<!--              <div @click="fabClick"-->
<!--                   v-permission="['PROJECT_API_SCENARIO:READ+EDIT', 'PROJECT_API_SCENARIO:READ+CREATE']">-->
<!--                <vue-fab id="fab" mainBtnColor="#63B8FF" size="small" :global-options="globalOptions"-->
...
<!--                            ref="scenarioRelevance"/>-->
        ...
        <el-drawer v-if="type!=='detail'" :visible.sync="debugVisible" :destroy-on-close="true" direction="ltr"
          ...
          <ms-api-report-detail :scenario="currentCase" :report-id="reportId" :debug="true"
...
import {
  getCurrentProjectID, getCurrentSubProjectID,
  getCurrentUser,
  getUUID,
  handleCtrlREvent,
  handleCtrlSEvent,
...
import {buttons, setComponent} from "@/business/components/api/automation/scenario/menu/Menu";

let jsonPath = require('jsonpath');
export default {
  name: "UiAutomationEditContainer",
  props: {
    moduleOptions: Array,
    currentCase: {},
    type: String,
  ...
  components: {
    ...
    ScenarioRelevance: () => import("@/business/components/api/automation/scenario/api/ScenarioRelevance"),
    ScenarioApiRelevance: () => import("@/business/components/api/automation/scenario/api/ApiRelevance"),
    ApiEnvironmentConfig: () => import("@/business/components/api/test/components/ApiEnvironmentConfig"),
    MsApiReportDetail: () => import("@/business/components/api/automation/report/SysnApiReportDetail"),
    MsInputTag: () => import("@/business/components/api/automation/scenario/MsInputTag"),
    MsRun: () => import("@/business/components/api/automation/scenario/DebugRun"),
    MsApiCustomize: () => import("@/business/components/api/automation/scenario/ApiCustomize"),
    ApiImport: () => import("@/business/components/api/definition/components/import/ApiImport"),
    ...
    MsApiVariableAdvance: () => import("@/business/components/api/definition/components/ApiVariableAdvance"),
  ...
  data() {
    return {
      ...
      scenarioDefinition: [],
      path: "/ui/scenario/create",
      debugData: {},
      reportId: "",
      enableCookieShare: false,
      enablePreScenario: false,
      globalOptions: {
        spacing: 30
      },
      response: {},
  ...
  created() {
    ...
    if (!this.currentCase.moduleId) {
      this.currentCase.moduleId = "";
    }
    if (this.currentCase.moduleId === 'default-module') {
      this.currentCase.moduleId = this.moduleOptions[0].id;
    }
    this.debug = false;
    ...
    if(this.currentCase.preScenarioId){
      this.enablePreScenario = true;
    }
  },
  mounted() {
    this.$nextTick(() => {
  ...
  methods: {
    clearPreScenario(isChecked){
      if(!isChecked){
        this.currentCase.preScenarioId = '';
      }
    },
    getScenarioList(){
      let subProjectId = '';
      ...
      if(projectId){
      this.$get('/ui/scenario/listAll/' + projectId + '/' + subProjectId, response => {
    ...
    getSubProjectsByProjectId(){
      ...
      if(projectId){
        this.$get('/project/getSubProjects/' + projectId, response => {
          ...
          if(subProjectId && !this.currentCase.subProjectId){
            this.currentCase.subProjectId = subProjectId;
          }
        })}},
    handleStep(com){
      this.buttonData = buttons(this);
      this.buttonData.forEach(button => {if(button.title === com){return button.click() }})
    },
    refreshApiScenario() {
    ...
    setDomain() {
      if (this.projectEnvMap && this.projectEnvMap.size > 0) {
        let scenario = {
          ...
          // preScenarioId: this.preScenarioId,
          name: this.currentCase.name,
          type: "UiScenario",
          clazzName: TYPE_TO_C.get("UiScenario"),
          variables: this.currentCase.variables,
          headers: this.currentCase.headers,
          referenced: 'Created',
          environmentMap: strMapToObj(this.projectEnvMap),
          hashTree: this.scenarioDefinition,
          onSampleError: this.onSampleError,
          projectId: this.currentCase.projectId ? this.currentCase.projectId : this.projectId,
        };
        let param = {
    ...
    initPlugins() {
      if (this.plugins) {
        this.plugins.forEach(item => {
          ...
          this.$nextTick(() => {
            if (!this.currentCase.name && this.$refs.refFab) {
    ...
    handleCommand() {
      ...
      /*触发执行操作*/
      this.$refs['currentCase'].validate(async (valid) => {
          if (valid) {
            this.debugLoading = true;
            let definition = JSON.parse(JSON.stringify(this.currentCase));
    ...
    openHis() {
      this.$refs.changeHistory.open(this.currentCase.id, ["接口自动化", "Api automation", "接口自動化", "API_AUTOMATION"]);
    },
    setModule(id, data) {
      this.currentCase.moduleId = id;
      this.currentCase.modulePath = data.path;
    },
    setHideBtn() {
    ...
    setPreScenario(id) {
      this.scenarioList.forEach(scenario => {
        if(scenario.id === id){
          this.currentCase.preScenarioName = scenario.name;
        }
      })
      this.$forceUpdate()
      // this.currentCase.preScenarioId = this.scenarioList[index].id;
      // this.currentCase.preScenarioName = this.scenarioList[index].name;
    },
    ...
    setVariables(v, headers) {
      this.currentCase.variables = v;
      this.currentCase.headers = headers;
    ...
    addScenario(arr) {
      if (arr && arr.length > 0) {
        arr.forEach(item => {
          if (item.id === this.currentCase.id) {
    ...
    addStep(){
      ...
      request.resourceId = getUUID();
      request.id = getUUID();
      request.index = this.scenarioDefinition.length + 1
      request.clazzName = 'com.supretest.api.dto.definition.request.sampler.StWebDriverSampler'
      request.requestResult = [];
      request.step = {"pageId":"",  "pageName":"", "elementId":"", "elementName":"","operation":"", "sendKeys":"" ,"waitTime":""}
      request.type = 'StWebDriverSampler';
      request.isLeaf = true;
      if (this.selectedTreeNode !== undefined) {
        this.selectedTreeNode.hashTree.push(request);
      } else {
    ...
    runDebug(runScenario) {
      ...
      this.$refs.currentCase.validate(async (valid) => {
        if (valid) {
          ...
          let scenarioDefinition_before = this.scenarioDefinition;
          if(this.currentCase.preScenarioId) {
            scenarioDefinition_before = await this.getPreScenario(scenarioDefinition_before, this.currentCase.preScenarioId)

          }
          //调试时不再保存
          this.debugData = {
            id: scenario ? scenario.id : this.currentCase.id,
            name: scenario ? scenario.name : this.currentCase.name,
            type: "UiScenario",
            variables: scenario ? scenario.variables : this.currentCase.variables,
            referenced: 'Created',
            headers: scenario ? scenario.headers : this.currentCase.headers,
            environmentMap: scenario && scenario.environmentEnable ? scenario.environmentMap : this.projectEnvMap,
            hashTree: scenario ? scenario.hashTree : scenarioDefinition_before,
            onSampleError: this.onSampleError,
          };
    ...
    editScenario() {
      ...
      return new Promise((resolve) => {
        ...
        if (this.$refs['currentCase']) {
          this.$refs['currentCase'].validate(async (valid) => {
            if (valid) {
              await this.setParameter();
              if (!this.currentCase.versionId) {
                if (this.$refs.versionHistory && this.$refs.versionHistory.currentVersion) {
                  this.currentCase.versionId = this.$refs.versionHistory.currentVersion.id;
                }
              }
              saveScenario(this.path, this.currentCase, this.scenarioDefinition, this, (response) => {
                this.$success(this.$t('commons.save_success'));
                this.path = "/ui/scenario/update";
                ...
                // 保存成功后刷新历史版本
                this.getVersionHistory();
                if (this.currentCase.tags instanceof String) {
                  this.currentCase.tags = JSON.parse(this.currentCase.tags);
                }
                if (this.currentCase.copy) {
                  this.currentCase.copy = null;
                }
                if (this.currentCase.type) {
                  this.currentCase.type = null;
                }
                this.pluginDelStep = false;
                // 记录改变后的数据数据
                let v1 = {
                  moduleId: this.currentCase.moduleId,
                  name: this.currentCase.name,
                  status: this.currentCase.status,
                  principal: this.currentCase.principal,
                  level: this.currentCase.level,
                ...
                this.currentCase.scenarioDefinitionOrg = v1;
                this.currentCase.scenarioDefinition = this.scenarioDefinition;
                this.$emit('refresh', this.currentCase);
                resolve();
              });
            }
          })
    ...
    getPreScenario(scenario,preScenarioId){
      return new Promise((resolve) => {
        this.$get('/ui/scenario/getUiScenario/preScenario/' + preScenarioId, response => {

          let obj = JSON.parse(response.data.scenarioDefinition).hashTree;
    ...
    getApiScenario(isRefresh) {
      ...
      this.isCheckedAll = false;
      this.selectDataCounts = 0;
      if (this.currentCase.tags !== undefined && this.currentCase.tags) {
        if (!(this.currentCase.tags instanceof Array)) {
          this.currentCase.tags = JSON.parse(this.currentCase.tags);
        }
      } else {
        this.$set(this.currentCase, 'tags', [])
      }
      if (!this.currentCase.variables) {
        this.currentCase.variables = [];
      }
      if (!this.currentCase.headers) {
        this.currentCase.headers = [];
      }
      if (this.currentCase && this.currentCase.id) {
        this.result = this.$get("/ui/scenario/getUiScenario/" + this.currentCase.id, response => {
          ...
          if (response.data) {
            ...
            if (response.data.scenarioDefinition != null) {
              ...
              if (obj) {
                ...
                this.currentCase.variables = [];
                if (obj.headers) {
                  this.currentCase.headers = obj.headers;
                }
                let index = 1;
                if (obj.variables) {
                  obj.variables.forEach(item => {
                    ...
                    if (item.name) {
                      ...
                      item.num = index;
                      this.currentCase.variables.push(item);
                      index++;
                    }
                  })
                }
                this.enablePreScenario = obj.preScenarioId? true: false;
                this.currentCase.preScenarioId = obj.preScenarioId;
                this.currentCase.preScenarioName = obj.preScenarioName;
            ...
            if (this.currentCase.copy) {
              this.path = "/ui/scenario/create";
            }
            this.$get('/api/automation/follow/' + this.currentCase.id, response => {
              this.currentCase.follows = response.data;
    ...
    initParameter() {
      this.currentCase.stepTotal = this.scenarioDefinition.length;
      if (!this.currentCase.projectId) {
        this.currentCase.projectId = this.projectId;
      }
      // 构建一个场景对象 方便引用处理
      let scenario = {
        id: this.currentCase.id,
        preScenarioId: this.currentCase.preScenarioId,
        name: this.currentCase.name,
        type: "UiScenario",
        clazzName: TYPE_TO_C.get("UiScenario"),
        variables: this.currentCase.variables,
      ...
      this.currentCase.environmentType = this.environmentType;
      this.currentCase.environmentJson = JSON.stringify(strMapToObj(this.projectEnvMap));
      this.currentCase.environmentGroupId = this.envGroupId;
      this.currentCase.scenarioDefinition = scenario;
      if (this.currentCase.tags instanceof Array) {
        this.currentCase.tags = JSON.stringify(this.currentCase.tags);
      }
      if (this.currentModule != null) {
        this.currentCase.modulePath = this.currentModule.method !== undefined ? this.currentModule.method : null;
        this.currentCase.moduleId = this.currentModule.id;
      }
    },
    async setParameter() {
      this.initParameter();
    ...
    showScenarioParameters() {
      this.$refs.scenarioParameters.open(this.currentCase.variables, this.currentCase.headers);
    },
    ...
    getVariableSize() {
      let size = 0;
      if (this.currentCase.variables) {
        size += this.currentCase.variables.length;
      }
      if (this.currentCase.headers && this.currentCase.headers.length > 1) {
        size += this.currentCase.headers.length - 1;
      }
      return size;
    },
    handleEnv() {
    ...
    showHistory() {
      this.$refs.taskCenter.openScenarioHistory(this.currentCase.id);
    },
    saveFollow() {
      if (this.showFollow) {
        this.showFollow = false;
        for (let i = 0; i < this.currentCase.follows.length; i++) {
          if (this.currentCase.follows[i] === this.currentUser().id) {
            this.currentCase.follows.splice(i, 1)
            break;
          }
        }
        if (this.currentCase.id) {
          this.$post("/api/automation/update/follows/" + this.currentCase.id, this.currentCase.follows, () => {
            this.$success(this.$t('commons.cancel_follow_success'));
          });
        }
      } else {
        this.showFollow = true;
        if (!this.currentCase.follows) {
          this.currentCase.follows = [];
        }
        this.currentCase.follows.push(this.currentUser().id)
        if (this.currentCase.id) {
          this.$post("/api/automation/update/follows/" + this.currentCase.id, this.currentCase.follows, () => {
    ...
    getVersionHistory() {
      this.$get('/api/automation/versions/' + this.currentCase.id, response => {
        if (this.currentCase.copy) {
          this.versionData = response.data.filter(v => v.versionId === this.currentCase.versionId);
        } else {
    ...
    compare(row) {
      this.scenarioRefId = this.currentCase.refId;
    ...
    checkout(row) {
      ...
      Object.assign(this.currentCase, api);
      this.getApiScenario();
      this.getVersionHistory();
    },
    create(row) {
      // 创建新版本
      this.currentCase.versionId = row.id;
      this.currentCase.versionName = row.name;
      this.loading = true;
      this.editScenario()
        .then(() => {
          this.loading = false;
    ...
    del(row) {
      this.$alert(this.$t('api_test.definition.request.delete_confirm') + ' ' + row.name + " ？", '', {
        ...
        callback: (action) => {
          if (action === 'confirm') {
            this.$get('/api/automation/delete/' + row.id + '/' + this.currentCase.refId, () => {
