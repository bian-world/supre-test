      ...
      <ms-table
        ...
        <span v-for="(item) in fields" :key="item.key">
          ...
          <ms-table-column
           ...
           <ms-table-column  prop="pagePath"
                            :field="item"
                            :fields-width="fieldsWidth"
                            :label="$t('ui.page_level')"
                            sortable
                            min-width="180px">
          <template v-slot:default="scope">
            <span v-for="(item) in scope.row.pagePath" :key="item.id">
           <el-link @click="edit(item.result)">{{ item.name }}</el-link> -
            </span>
          </template>
        </ms-table-column>
          ...
          <template v-slot:default="scope">
           ...
           <ms-table-column v-if="item.id != 'updateTime' && item.id != 'pagePath'" :prop="item.id"
                            :field="item"
                            :fields-width="fieldsWidth"
                            :label="item.label"
                            min-width="150px"/>
...
import {UI_PAGE_LIST} from "@/common/js/constants";
...
export default {
  ...
  data() {
    return {
      ...
      projectName: "",
      result: {},
      tableHeaderKey: "UI_PAGE",
      type: UI_PAGE_LIST,
      fields: getCustomTableHeader('UI_PAGE'),
  ...
  methods: {
    ...
    async search(projectId) {
      ...
      let url = "/ui/page/list/" + this.currentPage + "/" + this.pageSize;
      if (this.condition.projectId) {
        this.result = await this.$post(url, this.condition,  async response => {
          let data = response.data;
          this.total = data.itemCount;
          for(let page of data.listObject){
              if (page.pageLevel) {
                let levelList = page.pageLevel.split(",").slice(1)
                let list = []
                for (let parentId of levelList) {
                  let result =  await this.getParentPageInfo(parentId)
                  let name = ''
                  let page = {}
                  ...
                  list.push({'id': parentId, 'name': name, 'result': page})
                }
                this.pagePath = list
                page.pagePath = this.pagePath
                console.log('pagePath %o', this.pagePath)
                // page.pageLevel = list
              }
            }
          this.tableData = data.listObject
          console.log('tableData %o', this.tableData)
        });
      }
    },
    getParentPageInfo(parentId) {
      return new Promise((resolve) => {
        axios.get("/ui/page/get/" + parentId).then(res => {
            resolve(res.data)
          }
        )
      })
    ...
    addSon(row) {
      let data = JSON.parse(JSON.stringify(row));
      let sonData = {"parentId": data.id, "moduleId": data.moduleId, "subProjectId": data.subProjectId}
    ...
    remove(row) {
      if (!this.trashEnable) {
        this.$post('/ui/page/delete', {"id": row.id}, () => {
          this.$success(this.$t('commons.delete_success'));
          this.search();
        });
        return;
