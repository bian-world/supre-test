  ...
  <ms-container v-if="renderComponent" v-loading="loading">
    ...
    <ms-main-container style="overflow: hidden">
      <el-tabs v-model="activeName" @tab-click="addTab" @tab-remove="closeConfirm" ref="uiPage">
        <el-tab-pane name="default" :label="$t('ui.page_list')">
        <ui-page-list
        ...
        <el-tab-pane
          ...
          <div v-if="item.type === 'add' || item.type === 'edit'" class="ms-api-div">
            <page-edit-container
              :currentPage="item.currentPage"  :project-id="projectId" :syncTabs="syncTabs"  @savePage="savePage"
              :moduleOptions="moduleOptions" ref="pageEdit"/>
          </div>
        </el-tab-pane>
...
import UiPageList from "@/business/components/ui/uipage/UiPageList";
  ...
  methods: {
    ...
    addTab(tab) {
      ...
      if (tab.name === 'add') {
        ...
        currentPage = {...currentPage, ...tab.currentPage}
        if (this.nodeTree && this.nodeTree.length > 0) {
          currentPage.moduleId = this.nodeTree[0].id;
          this.getPath(this.nodeTree[0].id, this.moduleOptions);
          currentPage.modulePath = this.currentModulePath;
        }

        if (this.selectNodeIds && this.selectNodeIds.length > 0) {
          currentPage.moduleId = this.selectNodeIds[0];
          this.getPath(this.selectNodeIds[0], this.moduleOptions);
          currentPage.modulePath = this.currentModulePath;
        }
        this.tabs.push({label: label, name: name,currentPage: currentPage, type:'add',closable:true});
        console.log(this.tabs)
      }
      else if (tab.name === 'edit') {
        this.$get('/ui/page/releated/element/'+ tab.currentPage.id, response => {
          tab.currentPage.uiElementDTOS = response.data
          console.log(tab.currentPage)

        let name = getUUID().substring(0, 8);
        this.activeName = name;
        let label = tab.currentPage? tab.currentPage.name? tab.currentPage.name: this.$t('ui.add_page') : ''
        this.tabs.push({label: label, name: name, currentPage: tab.currentPage, type: 'edit', closable: true});
        });
      }
    ...
    // },
    editPage(row) {
      const index = this.tabs.find(p => p.currentPage.id === row.id && p.currentPage.copy === row.copy);

      if(!row.id && row.parentId){
        this.addTab({name: 'add', currentPage: row});
      }
      else if (!index) {
        this.addTab({name: 'edit', currentPage: row});
      }
      else  {
    ...
    savePage(data) {
      ...
      this.reqUrl = "/ui/page/edit/pageElements";
      this.$post(this.reqUrl, data,(response) => {
        this.$success(this.$t('commons.save_success'));
        if(response.data){
          this.tabs.forEach(tab => {
            if(tab.name == this.$refs.uiPage.currentName){
              tab.currentPage = response.data
            }
          })
          // this.$ref.pageEdit.forEach(page => {
          //   if(this.$ref.uiPage.currentName == page.)
          // })
          // this.$nextTick(() => { this.$refs.pageEdit[0].pageForm = response.data})
        }
        // this.currentApi.isCopy = false;
        // this.currentApi.sourceId = "";
        // // 创建了新版本的api，之后id变了，ref_id 保存了原始idpageForm
