<template>
  <div>
    ...
    <ms-node-tree
      ...
      :tree-nodes="data"
      :allLabel=" pageType == 'UIPage'? $t('ui.all_page') : $t('ui.all_case') "
...
<script>
  ...
  import {buildTree, buildNodePath} from "@/business/components/api/definition/model/NodeTree";
  ...
  export default {
    ...
    methods: {
      // handleImport() {
      //   if (this.projectId) {
      //     let subProjectId = this.subProjectId?this.subProjectId:'';
      //     this.result = this.$get("/api/automation/module/list/" + this.projectId + "/" + subProjectId, response => {
      ...
      list(projectId) {
        ...
        // } else
        if(this.pageType == 'UIPage') {
          if (this.isRelevanceModel) {
            url = "/ui/page/module/list/" + this.relevanceProjectId;
          } else {
            let subProjectId = this.subProjectId ? this.subProjectId : '';
            url = "/ui/page/module/list/" + (projectId ? projectId : this.projectId) + "/" + subProjectId;
            if (!this.projectId) {
              return;
            }
          }
        }else if(this.pageType == 'UIScenario'){
          let subProjectId = this.subProjectId ? this.subProjectId : '';
          url = "/ui/scenario/module/list/" + (projectId ? projectId : this.projectId) + "/" + subProjectId;
          if (!this.projectId) {
            return;
          }
        }
      ...
      edit(param) {
        ...
        this.$post("/ui/module/edit", param, () => {
      ...
      remove(nodeIds) {
        this.$post("/ui/module/delete", nodeIds, () => {
          this.list();
          this.refresh();
        }, (error) => {
          this.list();
      ...
      drag(param, list) {
        this.$post("/ui/module/drag", param, () => {
          this.$post("/ui/module/pos", list, () => {
            this.list();
          });
        }, (error) => {
          this.list();
