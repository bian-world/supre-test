      ...
      <div>
        <el-tabs v-model="activeName">
          <el-tab-pane :label="$t('schedule.task_config')" name="first">
            ...
            <div style="padding-top: 10px;" v-if="request.scenarioType!='UI'">
...
export default {
  ...
  methods: {
    ...
    updateTask(param) {
      this.result = this.$post('/api/schedule/updateEnableByPrimyKey', param, response => {
        ...
        if (this.paramRow.redirectFrom == 'testPlan') {
          ...
          if(this.request.scenarioType == 'UI'){
    ...
    open(row) {
      console.log('redirectFrom o%', row)
      //测试计划页面跳转来的
      let paramTestId = "";
      this.paramRow = row;
    ...
    saveSchedule() {
      ...
      if (this.schedule.enable) {
        if (this.request.scenarioType !='UI' && ((this.runConfig.environmentType === 'JSON' && Object.keys(this.runConfig.envMap).length === 0) || (this.runConfig.environmentType === 'GROUP' && !this.runConfig.environmentGroupId))) {
      ...
      if(this.request.scenarioType == 'UI'){
        url = '/ui/scenario/schedule/create';
      }
      if (this.scheduleTaskType === "TEST_PLAN_TEST") {
        ...
        if (param.id) {
          if(this.request.scenarioType == 'UI'){
            url = '/ui/scenario/schedule/update';
          }
          else{url = '/schedule/update';}
        }
      }
