...
<!--    </el-menu-item>-->
    <el-menu-item index="/dashboard" v-if="check('DashBoard')" onselectstart="return false"
    ...
    <el-menu-item index="/ui" @click="active()" v-if="check('ui')" onselectstart="return false"
                  v-permission="['PROJECT_API_SCENARIO:READ','PROJECT_UI_REPORT:READ']">
      {{ $t('commons.ui') }}
    </el-menu-item>
    ...
    <el-menu-item index="/plan" v-if="check('testplan')"
                  onselectstart="return false"
                  v-permission="['PROJECT_TRACK_CASE:READ','PROJECT_TRACK_PLAN:READ','PROJECT_TRACK_REVIEW:READ', 'PROJECT_TRACK_ISSUE:READ', 'PROJECT_TRACK_REPORT:READ']">
      {{ $t('commons.test_plan') }}
    </el-menu-item>
...
export default {
  ...
  methods: {
    ...
    handleSelect(index, path) {
      console.log("this.$route %o, index %s, path %s", this.$route, index, path)
    ...
    active() {
      if (this.activeIndex === '/api') {
        window.location.href = "/#/api/definition";
      }
    },
    check(key) {
